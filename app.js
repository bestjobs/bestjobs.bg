...
// 🛡️ Асинхронен AES-256-GCM декриптор (Нативно Web Crypto API)
...
async function initStaticQuiz() {
    ...
    // 🔄 Изчистване на остатъчните RAM данни от предходни опити
    ...
    // 🛡️ Прихващане на грешки при липсващ или дефектен файл в корена на главния сайт
    ...
    script.onload = async function() {
        ...
        // 🛡️ КОРЕКЦИЯ: Почистване на URL историята и закотвяне в корена на https://bestjobs.bg
        try {
        ...
        if (statusBox) {
            statusBox.innerHTML = '<div class="crypto-secure-badge">🛡️ Справочникът е декриптиран: AES-256-GCM (Локална RAM среда) | Локация: /</div>';
        }
    };
...
function buildQuizDOM() {
    ...
    for (var i = 0; i < activePool.length; i++) {
        ...
        // Фиксиран книжен ред на опциите без разместване
        ...
        if (q.k) {
            html += '<span class="btn-lbl" style="color:#3b82f6;" onclick="toggleS(this)">💡 Theorie</span><span class="btn-lbl" style="color:#e74c3c;" onclick="printL(' + i + ')">📥 PDF</span><div class="sol-box">' + q.s + '</div>';
        }
        html += '</div></div>';
    }
